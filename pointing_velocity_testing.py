...
async def experiment():
    ...
    global x, y, idx, run, missing, MAV, x_pos, y_pos
    ...
    while run:
        ...
        if np.sum(np.sum(channel)) != 0:
            ...
            if idx >= step + missing - 1:
                ...
                #vel_predict = one_euro_filter(currentTime, vel_predict)
        ...
        x_pos = x_pos + vel_predict[0] * 0.1
        y_pos = y_pos + vel_predict[1] * 0.1
        #y_pos = 0
...
with open('data/Adrian3/pointing_linear_pos_AMH.pkl', 'rb') as f:
    model = pickle.load(f)
...
target = circle.Circle(myWin, radius=0.3, lineColor=(1, 1, 1), fillColor=(0.75, 0, 0.75), autoDraw=True, pos=(0, 0))
